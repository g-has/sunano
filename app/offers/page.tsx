          ...
          <div className="mx-auto max-w-5xl px-4 py-6 md:px-6 lg:px-8 space-y-6">
            ...
            {loading ? (
              ...
              <div className="rounded-2xl border border-white/[0.08] bg-[#0d1117] p-10 text-center">
                <p className="text-slate-300">Nenhuma oferta disponível no momento.</p>
                    ...
                    >
                      ...
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {isExpiringSoon(offer.expires_at) && (
                            <Badge className="bg-red-500/20 text-red-300 hover:bg-red-500/20">Expirando</Badge>
                          )}
                        </div>
                      ...
                      <CardContent className="flex flex-1 flex-col space-y-4">
                        ...
                        <div className="space-y-1 text-xs text-slate-500">
                          <p>
                            Adicionada em {format(new Date(offer.created_at), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
                          </p>
                          {offer.expires_at && (
                            <p>
                        ...
                        {offer.peripheral && (
                          <div className="rounded-lg border border-white/[0.08] bg-white/[0.03] p-3">
                            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Periférico</p>
                            <p className="mt-1 text-sm font-semibold text-slate-200">
                              {offer.peripheral.brand} - {offer.peripheral.name}
                            </p>
                            <div className="mt-2 space-y-1 text-xs text-slate-400">
                              <p>
                                Performance: <span className="font-semibold text-slate-200">{offer.peripheral.tier}</span> ({formatModeValue(offer.peripheral.tags?.[0])})
                              </p>
                              <p>
                                Custo-Benefício: <span className="font-semibold text-slate-200">{offer.peripheral.tier}</span> ({formatModeValue(offer.peripheral.specs?.adminValueBand)})
                              </p>
                              <p>
                                Recomendado: <span className="font-semibold text-slate-200">{offer.peripheral.tier}</span> ({formatModeValue(offer.peripheral.specs?.adminRecommendedBand)})
                              </p>
                            </div>
                          </div>
                        )}
