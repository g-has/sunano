...
export default function EditPostClient({
  ...
  async function handleSave() {
    ...
    try {
      ...
      setError(message)
      setSaveStatus("error")
      toast.error("Erro ao salvar post", { description: message })
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleToggle(field: "is_hidden" | "is_locked" | "is_pinned") {
    ...
    try {
      ...
      const labels: Record<string, { on: string; off: string }> = {
        is_pinned: { on: "Post fixado", off: "Post desafixado" },
        is_hidden: { on: "Post oculto", off: "Post visível" },
        is_locked: { on: "Post bloqueado", off: "Post desbloqueado" },
      }
      toast.success(newValue ? labels[field].on : labels[field].off)
    } catch (err) {
