...
import { ArrowLeft, ShoppingCart, Package, Shield, AlertTriangle, ShoppingBag, Sparkles } from "lucide-react"
...
export default function BazarProductPage() {
  ...
  const [linkedStore, setLinkedStore] = useState<LinkedStoreProduct | null>(null)
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {linkedStore && (
                <Link
                  ...
                  className="group flex items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 transition hover:border-emerald-500/40 hover:bg-emerald-500/10"
                >
                  <div className="size-14 shrink-0 overflow-hidden rounded-lg border border-white/[0.08] bg-white/[0.04]">
                    {linkedStore.images?.[0] ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={linkedStore.images[0]} alt={linkedStore.name} className="h-full w-full object-contain p-1" />
                    ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-300">🛒 Comprar novo na Loja</p>
                    <p className="truncate text-sm font-semibold text-slate-100">{linkedStore.name}</p>
